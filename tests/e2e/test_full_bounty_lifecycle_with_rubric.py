...
Platform's `notify_bounty_funded(objective_schema=...)`) still has no owning service —
...
def test_full_lifecycle_from_free_text_description_to_payout(escrow_client, agent_client, rubric_client, oracle_client):
    ...
    draft_resp = rubric_client.post(
        ...
        json={
            "bounty_id": bounty_id,
            "bounty_description": "Find 100 ecommerce brands doing $1M-$25M in revenue",
    ...
    approve_resp = rubric_client.post(f"/rubrics/{bounty_id}/approve")
    assert approve_resp.json()["status"] == "approved"

    # 3. Requester funds the bounty via escrow.
    fund_resp = escrow_client.post(
        "/internal/escrow/fund",
        json={"bounty_id": bounty_id, "requester_id": "req-1", "amount_cents": 20_000, "take_rate_bps": 1000},
    ...
    lock_resp = rubric_client.post(f"/internal/rubrics/{bounty_id}/lock", headers=RUBRIC_INTERNAL_HEADERS)
    ...
    match_resp = agent_client.post(
        ...
        json={"bounty_id": bounty_id, "category": "sales_lead_generation", "objective_schema": schema_for_agent_platform},
    ...
    submit_resp = agent_client.post(
        "/submissions",
        json={"bounty_id": bounty_id, "payload": {"lead_count": 100}},
...
def test_rubric_cannot_be_edited_once_the_bounty_is_locked(rubric_client):
    bounty_id = "e2e-rubric-lock-guard"
    rubric_client.post(
        "/rubrics/draft",
        json={"bounty_id": bounty_id, "bounty_description": "find leads", "category": "sales_lead_generation"},
    )
    rubric_client.post(f"/rubrics/{bounty_id}/approve")
    rubric_client.post(f"/internal/rubrics/{bounty_id}/lock", headers=RUBRIC_INTERNAL_HEADERS)
...
def test_escrow_refuses_to_release_when_rubric_was_never_approved(escrow_client, rubric_client):
    ...
    bounty_id = "e2e-unapproved-rubric"
    rubric_client.post(
        "/rubrics/draft",
        json={"bounty_id": bounty_id, "bounty_description": "find leads", "category": "sales_lead_generation"},
    )
