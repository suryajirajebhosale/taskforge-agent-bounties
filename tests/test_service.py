...
def test_fund_bounty_creates_a_held_escrow(service):
    hold = service.fund_bounty(bounty_id="b1", requester_id="req1", amount_cents=5_000)
...
def test_funding_the_same_bounty_twice_is_idempotent(service, gateway):
    first = service.fund_bounty(bounty_id="b1", requester_id="req1", amount_cents=5_000)
    second = service.fund_bounty(bounty_id="b1", requester_id="req1", amount_cents=5_000)
...
def test_release_to_agent_pays_out_net_of_take_rate(service, db_session):
    service.fund_bounty(bounty_id="b1", requester_id="req1", amount_cents=10_000, take_rate_bps=1000)  # 10%

    payout = service.release_to_agent(bounty_id="b1", agent_developer_id="dev1")
...
def test_release_writes_take_rate_and_payout_ledger_entries(service, db_session):
    ...
    entries = db_session.query(LedgerEntry).filter_by(bounty_id="b1").all()
...
def test_release_with_zero_take_rate_does_not_write_a_zero_revenue_entry(service, db_session):
    service.fund_bounty(bounty_id="b1", requester_id="req1", amount_cents=10_000, take_rate_bps=0)
    service.release_to_agent(bounty_id="b1", agent_developer_id="dev1")

    entries = db_session.query(LedgerEntry).filter_by(bounty_id="b1", account="platform:revenue").all()
    assert entries == []
...
def test_double_release_is_idempotent_not_duplicated(service, gateway):
    ...
    first = service.release_to_agent(bounty_id="b1", agent_developer_id="dev1")
    second = service.release_to_agent(bounty_id="b1", agent_developer_id="dev1")
...
def test_refund_to_requester_returns_escrow(service):
    service.fund_bounty(bounty_id="b1", requester_id="req1", amount_cents=5_000)

    hold = service.refund_to_requester(bounty_id="b1")
...
def test_refund_writes_a_reversing_ledger_entry(service, db_session):
    service.fund_bounty(bounty_id="b1", requester_id="req1", amount_cents=5_000)
    service.refund_to_requester(bounty_id="b1")

    entries = db_session.query(LedgerEntry).filter_by(bounty_id="b1", account="requester:req1").all()
...
def test_double_refund_is_idempotent(service):
    ...
    first = service.refund_to_requester(bounty_id="b1")
    second = service.refund_to_requester(bounty_id="b1")
...
def test_release_on_unknown_bounty_raises_hold_not_found(service):
    with pytest.raises(HoldNotFound):
        service.release_to_agent(bounty_id="does-not-exist", agent_developer_id="dev1")


def test_refund_on_unknown_bounty_raises_hold_not_found(service):
    with pytest.raises(HoldNotFound):
        service.refund_to_requester(bounty_id="does-not-exist")
