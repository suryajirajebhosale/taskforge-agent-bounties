...
def test_generate_draft_creates_a_draft_record(service):
    record = service.generate_draft(job_id="b1", job_description="find leads", category=CATEGORY)
...
def test_generate_draft_passes_the_category_template_to_the_drafter(service, drafter):
    service.generate_draft(job_id="b1", job_description="find leads", category=CATEGORY)
    assert drafter.calls[0]["category"] == CATEGORY


def test_regenerating_a_draft_overwrites_the_previous_one(service, drafter):
    service.generate_draft(job_id="b1", job_description="v1", category=CATEGORY)
    ...
    service.generate_draft(job_id="b1", job_description="v2", category=CATEGORY)
...
def test_update_draft_replaces_the_requirement(service):
    ...
    service.update_draft(job_id="b1", requirement=edited)
...
def test_lock_is_idempotent(service):
    ...
    first = service.lock_for_funding(job_id="b1")
    second = service.lock_for_funding(job_id="b1")
