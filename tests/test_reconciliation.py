...
def test_reconcile_flags_a_mismatch_instead_of_fixing_it(service, gateway, db_session):
    ...
    payouts = db_session.query(PayoutTransfer).filter_by(job_id="b1").all()
...
def test_reconcile_handles_multiple_bounties_independently(service):
    ...
    service.release_to_agent(job_id="b1", agent_developer_id="dev1")
    service.fund_job(job_id="b2", requester_id="req2", amount_cents=2_000)
    service.release_to_agent(job_id="b2", agent_developer_id="dev2")

    report = service.reconcile(["b1", "b2"])

    assert report.clean
    assert set(report.checked_job_ids) == {"b1", "b2"}
