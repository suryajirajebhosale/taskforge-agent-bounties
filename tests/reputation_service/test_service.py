...
def test_record_outcome_creates_a_ledger_row(service):
    outcome = service.record_outcome(verdict_id="v1", agent_id="a1", agent_developer_id="d1", passed=True, bounty_amount_cents=1000)
    assert outcome.counted is True
    assert outcome.passed is True


def test_record_outcome_is_idempotent_per_verdict(service):
    service.record_outcome(verdict_id="v1", agent_id="a1", agent_developer_id="d1", passed=True, bounty_amount_cents=1000)
    replay = service.record_outcome(verdict_id="v1", agent_id="a1", agent_developer_id="d1", passed=False, bounty_amount_cents=999)

    assert replay.passed is True  # replay ignored, original stands
    assert replay.bounty_amount_cents == 1000
...
def test_get_rating_reflects_recorded_outcomes(service):
    service.record_outcome(verdict_id="v1", agent_id="a1", agent_developer_id="d1", passed=True, bounty_amount_cents=100)
    service.record_outcome(verdict_id="v2", agent_id="a1", agent_developer_id="d1", passed=True, bounty_amount_cents=100)
...
def test_get_rating_drops_after_a_string_of_fails(service):
    for i in range(5):
        service.record_outcome(verdict_id=f"v{i}", agent_id="a1", agent_developer_id="d1", passed=True, bounty_amount_cents=100)
    for i in range(5, 10):
        service.record_outcome(verdict_id=f"v{i}", agent_id="a1", agent_developer_id="d1", passed=False, bounty_amount_cents=100)
...
def test_correct_outcome_excludes_the_original_from_rating(service):
    ...
    service.correct_outcome(verdict_id="v1", agent_id="a1", agent_developer_id="d1", passed=False, bounty_amount_cents=100)
...
def test_correct_outcome_is_idempotent(service):
    service.record_outcome(verdict_id="v1", agent_id="a1", agent_developer_id="d1", passed=True, bounty_amount_cents=100)
    first = service.correct_outcome(verdict_id="v1", agent_id="a1", agent_developer_id="d1", passed=False, bounty_amount_cents=100)
    second = service.correct_outcome(verdict_id="v1", agent_id="a1", agent_developer_id="d1", passed=False, bounty_amount_cents=100)
...
def test_leaderboard_all_time_ranks_by_verified_earnings(service):
    service.record_outcome(verdict_id="v1", agent_id="a1", agent_developer_id="d1", passed=True, bounty_amount_cents=500)
    service.record_outcome(verdict_id="v2", agent_id="a2", agent_developer_id="d2", passed=True, bounty_amount_cents=1000)
    service.record_outcome(verdict_id="v3", agent_id="a3", agent_developer_id="d3", passed=False, bounty_amount_cents=99_999)
...
def test_leaderboard_weekly_only_counts_the_current_period(service):
    ...
    service.record_outcome(
        verdict_id="v1", agent_id="a1", agent_developer_id="d1", passed=True, bounty_amount_cents=500, occurred_at=OCCURRED_AT
    )
    service.record_outcome(
        verdict_id="v2", agent_id="a2", agent_developer_id="d2", passed=True, bounty_amount_cents=1000, occurred_at=last_week
    )
...
def test_finalize_weekly_prize_picks_the_top_developer(service):
    service.record_outcome(
        verdict_id="v1", agent_id="a1", agent_developer_id="d1", passed=True, bounty_amount_cents=1000, occurred_at=OCCURRED_AT
    )
    service.record_outcome(
        verdict_id="v2", agent_id="a2", agent_developer_id="d2", passed=True, bounty_amount_cents=500, occurred_at=OCCURRED_AT
    )
...
def test_finalize_weekly_prize_groups_earnings_by_developer_not_agent(service):
    ...
    service.record_outcome(
        verdict_id="v1", agent_id="a1", agent_developer_id="d1", passed=True, bounty_amount_cents=400, occurred_at=OCCURRED_AT
    )
    service.record_outcome(
        verdict_id="v2", agent_id="a1b", agent_developer_id="d1", passed=True, bounty_amount_cents=400, occurred_at=OCCURRED_AT
    )
    service.record_outcome(
        verdict_id="v3", agent_id="a2", agent_developer_id="d2", passed=True, bounty_amount_cents=700, occurred_at=OCCURRED_AT
    )
...
def test_finalize_weekly_prize_is_idempotent(service):
    ...
    service.record_outcome(
        verdict_id="v2", agent_id="a2", agent_developer_id="d2", passed=True, bounty_amount_cents=5000, occurred_at=OCCURRED_AT
    )
